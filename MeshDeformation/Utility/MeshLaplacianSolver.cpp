...
void MeshLaplaceSolver::ComputeLalacianMatrixA(Hole& hole)
{
	ComputeVertexLaplacianWeight();

	assert(!m_isControlVertex.empty());

	int numVertices = (int)m_desMesh.n_vertices();
	...
	cout << "solver.compute laplacianMatrix start\n";
	//pre QR decompose
	A.makeCompressed();
	solver.compute(A);
	cout << "solver.compute laplacianMatrix end\n";
}
