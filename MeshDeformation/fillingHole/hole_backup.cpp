...
double Hole::calcAngle(vec3 v1, vec3 v2, vec3 targetV, vec3 targetNormal)
{
	v1 -= targetV;
	v2 -= targetV;
...
void Hole::fillHoleWithDelaunayTriangulation(vector< pair<int, V2d<double> >> vecVIndexV2dPair)
{
	...
	{
		//if(min(edgeVector[i]., edgeVector[i].v2))
	}
...
void Hole::fillByAngle(vector<pair<TriMesh::VertexHandle, TriMesh::Point>> vecVH_PositionPair)
{
	...
	while (!vecVH_PositionPair.empty())
	{
		...
		if (nPrev == 0)
		{
			m_angles[nPrev] = calcAngle(toVec3(vecVH_PositionPair[vecVH_PositionPair.size() - 1].second), toVec3(vecVH_PositionPair[nNext].second), toVec3(vecVH_PositionPair[nPrev].second), toVec3(m_mesh->normal(vecVH_PositionPair[nPrev].first)));
		}
		else
		{
			m_angles[nPrev] = calcAngle(toVec3(vecVH_PositionPair[nPrev - 1].second), toVec3(vecVH_PositionPair[nNext].second), toVec3(vecVH_PositionPair[nPrev].second), toVec3(m_mesh->normal(vecVH_PositionPair[nPrev].first)));
		}

		if (nNext == vecVH_PositionPair.size() - 1)
		{
			m_angles[nNext] = calcAngle(toVec3(vecVH_PositionPair[nPrev].second), toVec3(vecVH_PositionPair[0].second), toVec3(vecVH_PositionPair[nNext].second), toVec3(m_mesh->normal(vecVH_PositionPair[nNext].first)));
		}
		else
		{
			m_angles[nNext] = calcAngle(toVec3(vecVH_PositionPair[nPrev].second), toVec3(vecVH_PositionPair[nNext + 1].second), toVec3(vecVH_PositionPair[nNext].second), toVec3(m_mesh->normal(vecVH_PositionPair[nNext].first)));
		}
