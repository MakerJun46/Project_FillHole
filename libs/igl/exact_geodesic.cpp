...
namespace igl {
	namespace geodesic {
		...
		{
			assert(p.size() % 3 == 0);
			unsigned const num_vertices = (unsigned const)p.size() / 3;
			assert(tri.size() % 3 == 0);
			unsigned const num_faces = (unsigned const)tri.size() / 3;
		...
		public:
			GeodesicAlgorithmExact(geodesic::Mesh* mesh) :
				...
				m_edge_interval_lists((unsigned int)mesh->edges().size())
			{
				m_type = EXACT;
